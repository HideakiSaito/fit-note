...
(function() {
  $(document).on("ready page:load", function() {
    var chart_home_toggle, chart_place_toggle, home_btn, place_btn;
    $('button').button();
    home_btn = $('#home-toggle');
    home_btn.button();
    chart_home_toggle = function() {
      return $('#chart_in_home').toggle('blind', '', 700);
    };
    home_btn.bind("click", chart_home_toggle);
    chart_home_toggle();
    place_btn = $('#place-toggle');
    place_btn.button();
    chart_place_toggle = function() {
      return $('#chart_pie_place').toggle('blind', '', 700);
    };
    place_btn.bind("click", chart_place_toggle);
    return chart_place_toggle();
  });

}).call(this);
(function() {
  $(function() {
    return $('input:submit').button();
  });
...
(function() {
  ...
  $(function() {
    $('#equipments').on('click', '.edit, .cancel', function(event) {
      return toggleEditor($(this).closest('.equipment'));
    }).on('ajax:complete', '.edit_equipment', function(event, ajax, status) {
      var $container, name, response;
      ...
      return toggleEditor($container);
    }).on('ajax:complete', '.delete_equipment', function(event, ajax, status) {
      alert(status);
      return $(this).closest('.equipment').remove();
    });
    return $('#new_equipment').on('ajax:complete', function(event, ajax, status) {
      var html, response;
      alert(status);
      response = $.parseJSON(ajax.responseText);
      html = response.html;
  ...
  toggleEditor = function($container) {
    ...
    if ($bodyField.is(':visible')) {
      return $bodyField.val($container.find('.viewr .body').text()).select();
    }
  };
...
(function() {
  $(document).on("ready page:load", function() {
    var change_page_copy_at, change_page_hidden_chart_at, change_page_hidden_lines_at;
    change_page_copy_at = function() {
      if ($("#page_is_copy").prop("checked")) {
        return $("#copy_page_id").show();
      } else {
        return $("#copy_page_id").hide();
      }
    };
    $("#page_is_copy").bind("click", change_page_copy_at);
    change_page_copy_at();
    change_page_hidden_lines_at = function() {
      if ($("#page_hidden_lines").prop("checked")) {
        $(".note").show();
        if ($(window).width() > 768) {
          return $(".note-index").css('height', '+=450');
        } else {
          return $(".note-index").css('height', 'auto');
        }
      } else {
        $(".note").hide();
        if ($(window).width() > 768) {
    ...
    return change_page_hidden_chart_at = function() {
      if ($("#page_hidden_chart").prop("checked")) {
        $(".chart").show();
        if ($(window).width() > 768) {
          return $(".note-index").css('height', '+=800');
        } else {
          return $(".note-index").css('height', 'auto');
        }
      } else {
